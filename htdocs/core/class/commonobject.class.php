...
abstract class CommonObject
{
	...
	/**
	 * Common function for all objects extending CommonObject for generating documents list
	 *
	 * @param 	string 		$modelspath 	Relative folder where generators are placed
	 * @param 	string 		$modele 		Generator to use. Caller must set it to obj->model_pdf or GETPOST('model_pdf','alpha') for example.
	 * @param 	Translate 	$outputlangs 	Output language to use
	 ...
	 * @see	addFileIntoDatabaseIndex()
	 */
	protected function commonGenerateDocumentList($modelspath, $modele, $outputlangs, $hidedetails, $hidedesc, $hideref, $moreparams = null)
	{
		global $conf, $langs, $user, $hookmanager, $action;
		...
		$parameters = array('modelspath'=>$modelspath, 'modele'=>$modele, 'outputlangs'=>$outputlangs, 'hidedetails'=>$hidedetails, 'hidedesc'=>$hidedesc, 'hideref'=>$hideref, 'moreparams'=>$moreparams);
		$reshook = $hookmanager->executeHooks('commonGenerateDocumentList', $parameters, $this, $action); // Note that $action and $object may have been modified by some hooks

		if (empty($reshook)) {
			dol_syslog("commonGenerateDocumentList modele=".$modele." outputlangs->defaultlang=".(is_object($outputlangs) ? $outputlangs->defaultlang : 'null'));
